...
router.delete("/delete/:commentID/", Authentication.isAuthenticated, async (req, res) => {
  let commentID = req.params.commentID;
  Comment.findById(commentID, async (err, comment) => {
    if (err) return res.status(500).send({ success: false, message: err.toString()});
    if (!comment) return res.status(404).send({ success: false, message: "Comment with Id " + commentID + " does not exist"});
    if (comment.poster != req.user._id && !Authentication.isInstructor) res.status(401).send({ success: false, message: "Comments can only be deleted by owner of post or original poster"});
    ...
    Comment.findByIdAndDelete(commentID, (err, delObj) => {
      return res.json({ success: true });
    });
  });
});
